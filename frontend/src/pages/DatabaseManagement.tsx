...
import { Database, FileText, Upload, Trash2, RefreshCw, Search, Loader2, ChevronDown, ChevronUp, CheckCircle, XCircle, ArrowRight, Info, Globe, Shield, Sparkles, Brain, Layers, Box, HardDrive, Zap, Cpu, Hash } from 'lucide-react';
...
const DatabaseManagement = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [sysInfo, status] = await Promise.all([api.ragSystemInfo(email), api.ragStatus()]);
    ...
    return (
      <Layout>
        <div className="flex items-center justify-center py-32">
          ...
          <span className="ml-3 text-slate-500">Memuat informasi database...</span>
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-700 mb-2">Kelola Database</h1>
          <p className="text-slate-500 text-sm">Manajemen dokumen, reindex, dan pengujian retrieval untuk admin IT manager.</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
          {[
            ...
            <div key={i} className="bg-white border border-slate-100 rounded-xl p-4 text-center">
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center mx-auto mb-2 ${s.color}`}><s.icon className="w-4 h-4" /></div>
          ...
          {expandedSections.documents && (
            <div className="px-4 sm:px-5 pb-5">
              <div className="flex flex-wrap gap-2 mb-4">
                <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50">
                  {uploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />}
                  {uploading ? 'Mengupload...' : 'Upload Dokumen'}
                </button>
                <input ref={fileInputRef} type="file" accept=".txt,.pdf,.json,.md,.csv,.docx" onChange={handleUpload} className="hidden" />
                <button onClick={handleReindex} disabled={reindexing} className="inline-flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-50">
                  {reindexing ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                  {reindexing ? 'Reindexing...' : 'Reindex Semua'}
                </button>
                <button onClick={loadData} className="inline-flex items-center gap-2 px-4 py-2 border border-slate-200 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors">
              ...
              {(rag.documents || []).length === 0 ? (
                ...
                <div className="border border-slate-100 rounded-lg overflow-hidden">
                  <table className="w-full text-sm">
                    <thead><tr className="bg-slate-50 text-left"><th className="px-4 py-2.5 text-xs font-semibold text-slate-500 uppercase">Nama File</th><th className="px-4 py-2.5 text-xs font-semibold text-slate-500 uppercase hidden sm:table-cell">Tipe</th><th className="px-4 py-2.5 text-xs font-semibold text-slate-500 uppercase hidden sm:table-cell">Ukuran</th><th className="px-4 py-2.5 text-xs font-semibold text-slate-500 uppercase hidden md:table-cell">Terakhir Diubah</th><th className="px-4 py-2.5 text-xs font-semibold text-slate-500 uppercase text-right">Aksi</th></tr></thead>
                    <tbody className="divide-y divide-slate-50">
                      {(rag.documents || []).map((doc: any) => (
                        <tr key={doc.name} className="hover:bg-slate-50/50">
                          <td className="px-4 py-3"><div className="flex items-center gap-2"><FileText className="w-4 h-4 text-slate-400 flex-shrink-0" /><span className="font-medium text-slate-700 truncate max-w-[200px]">{doc.name}</span></div></td>
                          <td className="px-4 py-3 hidden sm:table-cell"><span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-xs font-mono">{doc.extension}</span></td>
                          <td className="px-4 py-3 text-slate-500 hidden sm:table-cell">{doc.sizeFormatted}</td>
                          <td className="px-4 py-3 text-slate-400 text-xs hidden md:table-cell">{doc.lastModified ? new Date(doc.lastModified).toLocaleString('id-ID') : '-'}</td>
                          <td className="px-4 py-3 text-right"><button onClick={() => handleDelete(doc.name)} disabled={deleting === doc.name} className="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50">{deleting === doc.name ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-3 h-3" />}Hapus</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {expandedSections.testQuery && (
            <div className="px-4 sm:px-5 pb-5">
              <form onSubmit={handleTestQuery} className="flex gap-2 mb-4">
                <input type="text" value={testQuery} onChange={e => setTestQuery(e.target.value)} placeholder='Contoh: "jam buka puskesmas" atau "gejala demam berdarah"' className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2.5 text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent" />
                <button type="submit" disabled={querying || !testQuery.trim()} className="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50">
                  {querying ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}Cari
                </button>
              </form>
              {queryResults && (
                <div className="space-y-3">
                  <p className="text-xs text-slate-500">Query: <span className="font-medium text-slate-700">"{queryResults.query}"</span> — {queryResults.count} hasil</p>
                  {(queryResults.results || []).length === 0 ? (
                    ...
                    <div key={i} className="bg-slate-50 rounded-lg p-4 border border-slate-100">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-medium text-slate-500"><FileText className="w-3 h-3 inline mr-1" />{r.source}</span>
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${r.score >= 0.4 ? 'bg-emerald-100 text-emerald-700' : r.score >= 0.25 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600'}`}>Score: {r.score}</span>
                      </div>
